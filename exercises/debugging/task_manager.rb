class TaskManager
  ...
  def display_high_priority_tasks
    hp_tasks = tasks.select do |task| # error comes from using `tasks = tasks.select...` because `tasks =` is local variable assignment, and then the #select is called on it before it is assigned (it shadows @tasks)
      task.priority == :high # however, we don't want to actually reassign @tasks here either. so the trick is to just use a new name for this local variable before we display it
    end
