...
module Mailing
  ...
  def send_mail(destination, mail) # LS solution recommends renaming the method as well so we don't accidentally override an existing method name
...
class PostalService < CommunicationsProvider
  ...
  include Mailing # mixed in the module so johns_postal_service has access to the #send method defined above
...
# ruby thought we were trying to call methods using #send, which is a method that "sends" a method call to the calling object, with optional arguments.
# essentially, we were supposedly trying to call a method which doesn't exist, with the Postcard object as its argument!
